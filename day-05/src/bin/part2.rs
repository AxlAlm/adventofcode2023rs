...
fn find_lowest_location(almnac: &Almnac) -> u64 {
    ...
    for i in (0..almnac.seeds.len()).step_by(2) {
        // println!("--------");
        let seed_start = almnac.seeds[i];
        let seed_range = almnac.seeds[i + 1];
        for j in seed_start..seed_start + seed_range {
            // println!("_________");
            // dbg!(j);
            let location = map_src_to_dst(almnac, j.to_owned(), start_key);
